...
func (c *storedWorkflowIdentityCache) replace(runID string, identity storedWorkflowIdentity) {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (r *ResourceManager) CreateOrUpdateTasksForRun(
	...
	tasks, err := r.taskStore.CreateOrUpdateTasks(tasksToReport, runID)
	if err != nil {
...
func (r *ResourceManager) reportWorkflowResource(
	...
	if updateError == nil {
		...
		if verifiedExistingWorkflow != nil {
			...
			if !legacyIdentityAdoption {
				var err error
				workflowIdentityAdopted, err = r.validateStoredOrAdoptRetryWorkflowReportIdentity(run, verifiedExistingWorkflow)
				if err != nil {
					return nil, err
				}
			} else {
				workflowIdentityAdopted = true
			}
	...
	if updateError == nil && !createdFromRecurringReport {
		...
		if workflowIdentityAdopted {
			r.storedWorkflowIdentities.replace(runId, storedWorkflowIdentity{
