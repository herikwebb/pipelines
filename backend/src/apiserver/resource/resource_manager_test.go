...
func TestStoredWorkflowIdentityCacheReplacesAdoptedRetryIdentity(t *testing.T) {
	cache := storedWorkflowIdentityCache{}
	oldIdentity := storedWorkflowIdentity{name: "workflow", uid: "old-uid", retryGeneration: 1}
	adoptedIdentity := storedWorkflowIdentity{name: "workflow", uid: "new-uid", retryGeneration: 1}

	cache.loadOrStore("run", oldIdentity)
	cache.replace("run", adoptedIdentity)
	actual := cache.loadOrStore("run", oldIdentity)

	assert.Equal(t, adoptedIdentity, actual,
		"an overlapping report must not restore the pre-adoption identity from the same retry generation")
}
