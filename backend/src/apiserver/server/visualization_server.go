...
func (s *VisualizationServer) validateCreateVisualizationRequest(request *go_client.CreateVisualizationRequest) error {
	if request.Visualization.Type == go_client.Visualization_CUSTOM && !common.IsCustomVisualizationsAllowed() {
		return util.NewInvalidInputError(
			"Custom visualizations are disabled. Set ALLOW_CUSTOM_VISUALIZATIONS=true to enable them.",
		)
	}
