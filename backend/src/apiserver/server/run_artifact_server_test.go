...
func TestReadArtifact_MultiUserRejectsKeyOutsideRunNamespace(t *testing.T) {
	...
	runArtifactServer := NewRunArtifactServer(manager)
	req := httptest.NewRequest("GET", fmt.Sprintf("/apis/v2beta1/runs/%s/nodes/node-1/artifacts/artifact-1:read", run.UUID), nil)
...
func TestReadArtifact_MultiUserServesKeyInsideRunNamespace(t *testing.T) {
