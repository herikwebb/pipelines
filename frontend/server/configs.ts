...
export function loadConfigs(argv: string[], env: ProcessEnv): UIConfigs {
  ...
  let tensorboardProxySigningSecret = TENSORBOARD_PROXY_SIGNING_SECRET;
  if (!tensorboardProxySigningSecret) {
    tensorboardProxySigningSecret = randomBytes(32).toString('base64url');
    console.warn(
      'TENSORBOARD_PROXY_SIGNING_SECRET is not set; using an ephemeral per-process secret. ' +
        'Set TENSORBOARD_PROXY_SIGNING_SECRET for stable multi-replica deployments.',
    );
  }
  ...
  return {
    ...
    viewer: {
      tensorboard: {
        ...
        proxySigningSecret: tensorboardProxySigningSecret,
      },
    },
    visualizations: {
